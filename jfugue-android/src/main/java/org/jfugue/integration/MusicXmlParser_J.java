...
public final class MusicXmlParser_J extends Parser {
   ...
   * @param partHeader is the array of <code>XMLpart</code> classes that stores the
   ...
   * @param partHeaders is the array of XMLpart classes that contains instrument info
      ...
      if (part.getAttribute("id").getValue().equals(partHeaders[x].ID)) {
        ...
        if (partHeaders[x].midi_instruments[0] == null) { //if there are no midi instruments for the part ie the midi-instruments string length is 0
          ...
          if (partHeaders[x].midi_instruments[0][1] != null)
            parseVoice(p, Integer.parseInt(partHeaders[x].midi_instruments[0][1]));
